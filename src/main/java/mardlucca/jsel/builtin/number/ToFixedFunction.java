...
public class ToFixedFunction extends JSELFunction {
    public static final String NAME = "toFixed";
    private static final int MAX_DIGITS = 20;

    public ToFixedFunction() {
        super(NAME, Collections.singletonList("fractionDigits"));
    }
    ...
    public JSELValue call(
        ...
        int lFractionDigits = getArgument(aInArguments).toInteger();

        if (lFractionDigits < 0 || lFractionDigits > MAX_DIGITS) {
            throw rangeError(
        ...
        return new JSELString(DecimalFormat.format(lNumber, lFractionDigits));
    }
}
